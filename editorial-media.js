(function(){
 ...
 function decoratePeriods(){const box=document.getElementById('periods');if(!box)return;const data=current();[...box.querySelectorAll('.period')].forEach((card,i)=>{card.querySelector('.period-media')?.remove();const d=data[i];if(!d)return;const m=document.createElement('div');m.className='period-media';m.innerHTML='<img loading="lazy" src="'+d[0]+'" alt=""><span>'+d[1]+'</span>';card.prepend(m)})}
 function decorateStory(){const data=current();document.querySelectorAll('.story2500-step').forEach((card,i)=>{if(card.querySelector('.story2500-media'))return;const d=data[i];if(!d)return;const media=document.createElement('div');media.className='story2500-media';media.innerHTML='<img loading="lazy" src="'+d[0]+'" alt="">';const wrap=document.createElement('div');wrap.className='story2500-body';while(card.firstChild)wrap.appendChild(card.firstChild);card.append(media,wrap)})}
 function detailForNode(node){if(!node)return west[0];const rect=node.getBoundingClientRect(),sc=document.querySelector('.scroll'),max=Math.max(1,sc?.scrollWidth||1),logical=(sc?.scrollLeft||0)+rect.left;const i=Math.max(0,Math.min(9,Math.round(logical/292)));return west[i]}
 ...
 const obs=new MutationObserver(()=>{decoratePeriods();decorateStory()});function init(){if(!document.getElementById('periods'))return requestAnimationFrame(init);decoratePeriods();decorateStory();obs.observe(document.body,{childList:true,subtree:true});document.getElementById('westBtn')?.addEventListener('click',()=>setTimeout(()=>{decoratePeriods();decorateStory()},140));document.getElementById('eastBtn')?.addEventListener('click',()=>setTimeout(()=>{decoratePeriods();decorateStory()},140))}init();
})();
