(function(){
 ...
 function decorate(){const card=document.querySelector('.v5-card.open');if(!card)return;const h=card.querySelector('h2');if(!h)return;const url=resolve(h.textContent);let existing=card.querySelector('[data-literary-atlas]');if(!url){existing?.remove();return}if(existing){existing.href=url;return}const a=document.createElement('a');a.href=url;a.target='_self';a.rel='noopener';a.dataset.literaryAtlas='1';a.className='literary-atlas-link';a.innerHTML='<span class="literary-atlas-k">GÖRSEL ATLAS</span><span class="literary-atlas-copy"><b>Bu eseri bağlamıyla keşfet</b><em>Haritalar, karakterler ve fikir katmanlarıyla.</em></span><i>→</i>';const identity=card.querySelector('.v4-identity');(identity||h).insertAdjacentElement('afterend',a)}
 const css=document.createElement('style');css.textContent='.literary-atlas-link{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:12px;margin:12px 0 18px;padding:13px 14px;border:1px solid color-mix(in srgb,var(--gold) 45%,var(--border));background:linear-gradient(180deg,color-mix(in srgb,var(--gold) 6%,var(--card)),var(--card));text-decoration:none;color:var(--text);transition:.2s}.literary-atlas-k{font:500 9px var(--font-mono);letter-spacing:.16em;color:var(--gold);white-space:nowrap}.literary-atlas-copy{display:block;min-width:0}.literary-atlas-copy b{display:block;font:500 14px/1.25 var(--font-display);margin-bottom:2px}.literary-atlas-copy em{display:block;font:11px/1.35 var(--font-body);font-style:normal;color:var(--text-sec)}.literary-atlas-link i{font-style:normal;color:var(--gold);font-size:18px;transition:transform .2s}.literary-atlas-link:hover{border-color:var(--gold);background:color-mix(in srgb,var(--gold) 10%,var(--card));transform:translateY(-1px)}.literary-atlas-link:hover i{transform:translateX(3px)}@media(max-width:520px){.literary-atlas-link{grid-template-columns:1fr auto;gap:8px}.literary-atlas-k{grid-column:1/-1}.literary-atlas-copy b{font-size:13px}.literary-atlas-copy em{font-size:10px}}';document.head.appendChild(css);
